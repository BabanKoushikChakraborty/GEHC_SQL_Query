select
    ...
    (select * from (select fpov.profile_option_value from APPS.fnd_profile_option_values fpov where fpov.level_value=fu.user_id order by fpov.last_update_date desc)  where rownum <2) "User Type",
