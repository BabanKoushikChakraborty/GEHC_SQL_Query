...
,(select distinct ipp.approval_code 
  ...
  to_date((select min(action_date) from apps.po_action_history where object_id = PRHA.REQUISITION_HEADER_ID and  object_type_code = 'REQUISITION' and action_code ='SUBMIT'), 'DD-MON-YY') "Req_Submit_Date",
   to_date((select min(action_date) from apps.po_action_history 
    ...
    to_date(ph.creation_date, 'DD-MON-YY')-to_date((select min(action_date) from apps.po_action_history 
    where object_id = PRHA.REQUISITION_HEADER_ID 
...
AND PH.ORG_ID in ('9183')



and ph.closed_code = 'OPEN'
 
AND TRUNC(PH.CREATION_DATE) BETWEEN  '01-JAN-2021' AND '01-JAN-2021' 
