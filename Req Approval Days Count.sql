SELECT 
    ...
    ,to_date((select max(action_date) from apps.po_action_history 
    where object_id = PRHA.REQUISITION_HEADER_ID 
    and object_type_code = 'REQUISITION' and action_code ='APPROVE'), 'DD-MON-YY') "Req_Approve_Date",
    to_date((select max(action_date) from apps.po_action_history 
    where object_id = PRHA.REQUISITION_HEADER_ID 
    and object_type_code = 'REQUISITION' and action_code ='APPROVE'), 'DD-MON-YY')-to_date((select min(action_date) from apps.po_action_history 
    where object_id = PRHA.REQUISITION_HEADER_ID 
    and object_type_code = 'REQUISITION' and action_code ='SUBMIT'), 'DD-MON-YY') "Days taken for Req approval",
    to_date(pha.creation_date, 'DD-MON-YY')-to_date((select min(action_date) from apps.po_action_history 
    where object_id = PRHA.REQUISITION_HEADER_ID 
    and object_type_code = 'REQUISITION' and action_code ='SUBMIT'), 'DD-MON-YY') "Difference PO_Gen and Req_Sub"
    
    ,to_date(prha.creation_date, 'DD-MON-YY') "Requisition Creation Date"
    
    ,prha.authorization_status "PR Status"
    ,to_date(prla.need_by_date, 'DD-MON-YY') "REQ Need by Date"
    ,to_date(pll.need_by_date, 'DD-MON-YY') "PO Need by Date"
    ,to_number(pha.segment1)  "PO number"
    ,pha.revision_num "REVISION_NUM"
    ,prda.attribute15 "Sales Order No"
    ...
    ,mc.description "Commodity Description"
    ...
    , (select distinct ipp.approval_code 
            ...
            and rownum<2)  as "QMS APPROVAL CODE"
    ...
    ,pda.distribution_num  "PO_Distribution_Line"
    ...
    ,(select  listagg('Name: '||papf.full_name||'; SSO: '||papf.employee_number||'; Role: '||apll.attribute5||'; Action Date: '||apll.response_date||';'||apll.attribute8||';'||
        to_char(trunc(apll.response_date)-trunc((select poah.action_date from apps.po_action_history poah where  
            poah.object_id = PRHA.REQUISITION_HEADER_ID and  poah.object_type_code = 'REQUISITION' and poah.sequence_num=apll.sequence_num))) ,' days | '
                    )
    ...
    , pv.segment1 GSL,
    (select location_code from hr_locations_all where location_id = PLL.ship_to_location_id) ship_to_location,
    (select location_code from hr_locations_all where location_id = PVSA.bill_to_location_id) bill_to_location
    ...
    and  TRUNC(prha.creation_date) BETWEEN '01-JAN-2021' and '05-JAN-2021'