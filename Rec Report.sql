  ...
  ,to_char(prla.need_by_date,'DD-MON-YY') "Need by Date"
...
,papf2.employee_number "Receiver SSO"
  ...
  ,round((pda.quantity_ordered),2) "Quantity ordered"
...
(nvl(pda.quantity_ordered,0)*pll.Price_Override) 
else 
(select (nvl(pda.quantity_ordered,0)*pll.Price_Override)* 
gdr.conversion_RATE 
...
and TO_CURRENCY = 'USD') 
  ...
  ,(select sum((nvl(pl.quantity,0)*pll.Price_Override)) from apps.po_line_locations_all pll,apps.po_lines_all pl
  where pl.po_line_id=pll.po_line_id and pha.po_header_id = pl.po_header_id)  "PO_Tot_AMT"
  
  ,ROUND((case 
when pha.currency_code = 'USD' then 
(select sum((nvl(pl.quantity,0)*pll.Price_Override)) from apps.po_line_locations_all pll,apps.po_lines_all pl where pl.po_line_id=pll.po_line_id and pha.po_header_id = pl.po_header_id )
else 
(select sum(nvl(pl.quantity,0)*pll.Price_Override*gdr.conversion_RATE) 
from apps.gl_daily_rates gdr ,apps.po_line_locations_all pll,apps.po_lines_all pl where pl.po_line_id=pll.po_line_id and pha.po_header_id = pl.po_header_id and to_char(trunc(gdr.CONVERSION_DATE), 'DD-MON-YYYY') = 
to_char(pha.creation_date,'DD-MON-RRRR') 
and upper(conversion_type) = upper('Corporate') 
and FROM_CURRENCY = pha.currency_code 
and TO_CURRENCY = 'USD' )end),2) "Usd PO_Tot_Amt" 
...
-- , (SELECT currency_code
 ...
 --   WHERE set_of_books_id = hro.set_of_books_id) FUNC_CURRENCY_CODE
   ...
   ,to_char(rsh.creation_date,'DD-MON-YY')   "Receipt date"
   ,rsh.packing_slip "Packing Slip"
   ,rsh.comments "Receipt Comments"
 , to_char(TRUNC(rt.transaction_date),'DD-MON-YY') Delivery_date
...
and  TRUNC(RSH.creation_date) BETWEEN '01-JAN-2018' and '31-DEC-2018'
